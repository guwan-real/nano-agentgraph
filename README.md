...
**Status:** Alpha. Built primarily for personal learning, reading, and
...
A tiny, readable LangGraph-style graph runtime for learning, hacking, and
...
It is primarily a personal learning project: a compact runtime for reading the
core ideas, trying small examples, and experimenting with agent graph mechanics.

Why use it:

- Keep the LangGraph shape, change the import prefix.
- Read the whole runtime without digging through a production framework.
- Use plain Python with no LangChain or LangGraph dependency.
- Get clear errors for unsupported advanced features.
...
| `from langgraph.prebuilt import ToolNode, tools_condition` | `from nano_agentgraph.prebuilt import ToolNode, tools_condition` |
...
- `tests/test_runtime_edges.py` covers limits and routing errors.
- `tests/test_prebuilt_tool_node.py` covers tool helpers.
...
一个小而可读的 LangGraph 风格图运行时，用来学习、实验和理解 agent 编排。
...
它首先是一个个人学习项目：用尽量小的实现读懂核心思路、跑小例子，并实验 agent
graph 的基础机制。

适合它的场景：

- 保留 LangGraph 的代码形状，只改导入前缀。
- 不翻大型生产框架，也能读懂运行时。
- 不依赖 LangChain 或 LangGraph，保持 plain Python。
- 对未支持的高级功能给出清晰错误。
...
- `tests/test_runtime_edges.py` 覆盖执行上限和路由错误。
- `tests/test_prebuilt_tool_node.py` 覆盖工具 helper。
